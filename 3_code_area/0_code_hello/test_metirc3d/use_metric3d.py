...
def use_metric3d(rgb_file_path):
    ...
    rgb_origin = cv2.imread(rgb_file_path)[:, :, ::-1]
        # 调整输入图像大小以适应预训练模型
    rgb, pad_info, intrinsic = main.adjust_input_size(image=rgb_origin, intrinsic=intrinsic)
        # 归一化
    rgb = main.normalize(rgb)
    ...
    # post processing
    pred_depth = main.unpad(pred_depth, rgb_origin, pad_info)
    pred_depth = main.decanonical_transform(intrinsic=intrinsic, pred_depth=pred_depth)
