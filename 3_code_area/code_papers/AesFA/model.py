# 本文件使用了network文件中的部分, 请先阅读同目录下的network.py文件
# networks.py文件中定义了AesFA网络的编码器与生成器, 而本文件则注重使用networks.py中定义的网络模块进行网络搭建
...
class AesFA(nn.Module):
    # 从命名上来看, 这应该是构建整个网络的类
    # 这个类是专门用于train.py文件使用的, 是用于训练的类
    def __init__(self, config):
        super(AesFA, self).__init__()

        ######从传入的config参数中获取一些参数设定######
        self.config = config
        ...
        ######定义AesFa网络中的内容图像编码器、风格图像编码器与Generator################
...
class AesFA_test(nn.Module):
    # 这个类是专门用于test.py文件使用的, 是用于测试的类
    # 由于本文使用的测试数据是直接从文章对应的github中下载的, 所以与train.py中的使用方法有区别
    def __init__(self, config):
